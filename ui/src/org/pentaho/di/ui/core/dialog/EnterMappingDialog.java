...
public class EnterMappingDialog extends Dialog
{
    ...
    public static final String STRING_SFORCE_RELATIONSHIP_FIELD_SUFFIX = "__R";

    public static final String STRING_SFORCE_CUSTOM_FIELD_SUFFIX = "__C";
      ...
      while ( !found && ( length >= ((int)(sourceString.length() * 0.85)) || first ) ) {
        ...
        for ( int i = 0; i < wTarget.getItemCount() && !found; i++ ) {
      	...
      	if (test.contains(EnterMappingDialog.STRING_SFORCE_EXTERNALID_SEPARATOR) ) {
      		...
      		if (test.endsWith(EnterMappingDialog.STRING_SFORCE_RELATIONSHIP_FIELD_SUFFIX)) {
      			test = test.substring(0,test.length()-3) + EnterMappingDialog.STRING_SFORCE_CUSTOM_FIELD_SUFFIX;
      		}
      	}
